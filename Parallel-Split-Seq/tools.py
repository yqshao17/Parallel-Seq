...
def split_attributes(s):
    ...
    # for hg19.gtf, remove _x
    att_values[0] = att_values[0].split('_')[0]
...
def make_gtf_annotations(species, gtf_filenames, output_dir, splicing):
    ...
    for s in species:
        ...
        gene_id_to_genome['Ambiguous_'+s]=s
...
def run_star(genome_dir, output_dir, sample, nthreads, PE):
    ...
    if PE==True:
        ...
        rc = subprocess.call(STAR_PATH + """ --genomeDir {0}/ --runThreadN {2} --readFilesIn {4}/{3}_barcode_R1.fq --outFileNamePrefix {1}/{3}. --limitOutSJcollapsed 5000000""".format(genome_dir, map_dir, nthreads, sample, sample_dir), shell=True) # error encountered when --limitOutSJcollapsed is default 1000000
