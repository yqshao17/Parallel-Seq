...
def get_shift_bc_info(bc1_part, bc1_shift, bc1_edit_dict_part,pos_cfg,bc_edit_dist1=1):
    ...
    if len(bc1_matches)==1:
        ...
        umi_bc_starts2[3]=umi_bc_starts2[3][shift]
...
def preprocess_fastq_chunk(output_dir, prefix, bcset, bc4, counts, config, return_dict, chunk=None):
    ...
    with open(fastq1,'r') as f1, open(fastq2,'r') as f2, \
        ...
        while True:
            ...
            #umi_bc_len,umi_bc_starts,tag_start = pos_cfg
            bc1_part = seq2[umi_bc_starts[1]:umi_bc_starts[1]+umi_bc_len[1][0]]
            ...
            if len(shift_bc_info)==3:
                ...
                bc1 = seq2[umi_bc_starts2[1]:umi_bc_starts2[1]+umi_bc_len2[1]]
                bc2 = seq2[umi_bc_starts2[2]:umi_bc_starts2[2]+umi_bc_len2[2]]
                bc3 = seq2[umi_bc_starts2[3]:umi_bc_starts2[3]+umi_bc_len2[3]]
                umi = seq2[umi_bc_starts2[0]:umi_bc_starts2[0]+umi_bc_len2[0]]
                ...
                umi_bc_starts2[3]=umi_bc_starts2[3][0]
            ...
            if bc1:
                if len(seq2)<tag_start2+tag_length+2: # \n is 1 length
                    seq2=seq2.rstrip('\n')+'N'*(tag_start2+tag_length+2-len(seq2))+'\n'
                    qual2=qual2.rstrip('\n')+qual2.rstrip('\n')[-1]*(tag_start2+tag_length+2-len(qual2))+'\n'
                tag_readcut = seq2[tag_start2:(tag_start2+tag_length)]
                ...
                if tag=='A':
                    atac_reads += 1
                    fout2.write(seq2[(tag_start2+tag_length):])
                    fout2.write(strand2)
                    fout2.write(qual2[(tag_start2+tag_length):])
                elif tag=='R':
                    rna_reads += 1
                    fout2.write(seq2[tag_start2:])
                    fout2.write(strand2)
                    fout2.write(qual2[tag_start2:])
                elif tag=='W':
                    waste_reads +=1
                    fout2.write(seq2[(tag_start2+tag_length):])
                    fout2.write(strand2)
            ...
            bc1_Q30_sum += np.mean([ord(c)>62 for c in qual2[umi_bc_starts2[1]:umi_bc_starts2[1]+umi_bc_len2[1]]])
            bc2_Q30_sum += np.mean([ord(c)>62 for c in qual2[umi_bc_starts2[2]:umi_bc_starts2[2]+umi_bc_len2[2]]])
            bc3_Q30_sum += np.mean([ord(c)>62 for c in qual2[umi_bc_starts2[3]:umi_bc_starts2[3]+umi_bc_len2[3]]])
            umi_Q30_sum += np.mean([ord(c)>62 for c in qual2[umi_bc_starts2[0]:umi_bc_starts2[0]+umi_bc_len2[0]]])
            # Make sure R2 length > (bc_starts[3]+umi_bc_len[3])
            R2_cDNA_Q30_sum += np.mean([ord(c)>62 for c in qual2[umi_bc_starts2[3]+umi_bc_len2[3]:-1]])
...
def preprocess_fastq(output_dir, prefix, bcset, bc4, config, nthreads):
        ...
        with gzip.open(fastq2,'rb') as f:
            while n<n_reads:
                ...
                if len(shift_bc_info)==3: # bc1_part is matched
                    ...
                    bc3 = seq[umi_bc_starts2[3]:umi_bc_starts2[3]+umi_bc_len2[3]]
    ...
    fastq2=output_dir+'/trimmed/'+prefix+'_R2.fq.gz'
    counts, perfect_rate= get_perfect_bc_counts(fastq2, pos_cfg,bc1_edit_dict,bc2_edit_dict,bc3_edit_dict)
...
if __name__=='__main__':
    ...
    output_dir='/Share2/home/zhangqf5/yanqiu/scAR/output/test_set2'
    prefix='test'
    #prefix='W56'
    bcset='Data/barcodes/bcset0_shift'
    bc4='testxxx.'
    config_file='config_10nt_shift'
    nthreads=2
    #nthreads=2
